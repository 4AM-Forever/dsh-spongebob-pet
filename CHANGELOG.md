...
- 桌宠端到端 **29/29**：`powershell -ExecutionPolicy RemoteSigned -File tools/e2e-pet-features.ps1`
  ...
  - 覆盖：多会话气泡文案、点桌宠弹出会话面板（窗口数 +1、面板显示会话名且不含会话 id、日志留痕）、再点一下收起、挪动时面板自动收起、**拖到左缘吸附（窗口 X 停在 -18、`dock=left`）与拖开后解除**、**副屏不被拽回主屏**、**丢出屏幕能自己拉回**、`celebrate` 四段庆祝跑完且实测时长对得上（4,014ms / 配置 4,000ms）、运行期无未捕获异常。用例失败时自动保留临时目录，里面有完整 `pet.log`。
