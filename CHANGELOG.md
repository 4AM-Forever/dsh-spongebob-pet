...
在 0.1.0 基础上补齐桌宠本体的观感与桌面适配：干完活分四段跳舞庆祝、多会话同时烧脑时点桌宠看会话面板、贴屏幕边缘自动吸附成趴墙探头姿态、多显示器各自就位不被拽回主屏；并修掉四个真机问题（吸在边缘时窗口裁身体、结束桌宠后设置页仍显示运行中、会话面板每行渲染崩、摆在副屏被定时拽回主屏）。

### 升级

```powershell
dsh plugin --profile web add github:4AM-Forever/dsh-spongebob-pet#v0.2
# 重启 dsh web 后：设置 → 插件 → 海绵宝宝桌宠
```
...
- **贴边吸附 + 趴墙探头姿态**：往屏幕边缘拖，松手时离边缘 18 DIU 以内即吸附到该边，并换成「趴在墙角探头看我们」的姿态——画布外层加了一层姿态变换（`Art.ps1` 的 Dock 层，与内部的呼吸/蹦跳 `BobTransform` 互不干扰），按边倾斜（左 `+16°` / 右 `-16°` / 上 `-7°` / 下 `0°`），身体往边缘外平移让窗口压在边缘上（左右各探出 14 DIU、上方 16 DIU），站姿朝屏幕内侧倾斜，并像挂在墙上一样以 ±3° 轻晃。拖动时自动摆正，松手按落点决定吸附还是解除；吸附状态落在 `pet/config.json` 的 `dock`，重启后接着吸。
  - **倾斜时按外接矩形补窗口余量**：绕中心转 θ 后的外接矩形比 220×284 画布大（16° 约 290×334），窗口不补就会出现「斜着身子但头/手臂被裁掉」。吸附时窗口按当前档位加上 `2×Get-DockPadding(角度+4°)` 的透明留白（小档 110×142 → 160×192），解除时缩回原尺寸并把左上角补回同样多的量，位置不会跳。
...
- 桌宠端到端 **31/31**：`powershell -ExecutionPolicy RemoteSigned -File tools/e2e-pet-features.ps1`
  - 做法：把 `pet\` 复制到临时目录，用 `test/stub-bridge.mjs` 顶替宿主（`DSH_HOME` 指向临时目录、`SBP_SINGLETON` 换掉单实例锁名），全程不碰正在运行的那只桌宠；断言只认 `pet.log`、真实窗口数与临时副本里的 `config.json`。脚本另起一个独立看门狗，保证临时副本最多存活 5 分钟。
  - 覆盖：多会话气泡文案、点桌宠弹出会话面板（窗口数 +1、面板显示会话名且不含会话 id、日志留痕）、再点一下收起、挪动时面板自动收起、**拖到左缘吸附（小档窗口由 110 撑到 160、`dock=left`）与拖开后缩回并解除**、**副屏不被拽回主屏**、**丢出屏幕能自己拉回**、`celebrate` 四段庆祝跑完且实测时长对得上（4,014ms / 配置 4,000ms）、运行期无未捕获异常。用例失败时自动保留临时目录，里面有完整 `pet.log`。
