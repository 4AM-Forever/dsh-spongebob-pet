...
const PET_CONFIG = join(PET_DIR, 'config.json')
...
export function apply(ctx, config = {}) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    const record = sessions.get(id) ?? { id, title: null, state: 'idle', updatedAt: 0 }
    ...
    switch (event.type) {
      ...
      case 'tool/result':
        ...
        break
      case 'turn/end':
        record.state = event.data?.reason?.kind === 'error' ? 'error' : 'idle'
        break
      default:
        break
    }
    ...
    if (before !== record.state) push('session', record)
  ...
  function stopPet() {
    const killed = petRunning() ? petChild.pid : null
    petChild = null
    try {
  ...
  function eventsRoute(req, res, url) {
    ...
    waiter.timer = setTimeout(() => flush(true), options.longPollMs)
    waiter.timer.unref?.()
    waiters.add(waiter)
    req.on('close', () => {
  ...
  function readPetSize() {
    try {
      const size = JSON.parse(readFileSync(PET_CONFIG, 'utf8')).size
  ...
  function writePetSize(size) {
    ...
    try {
      const config = existsSync(PET_CONFIG) ? JSON.parse(readFileSync(PET_CONFIG, 'utf8')) : {}
      config.size = size
      writeFileSync(PET_CONFIG, `${JSON.stringify(config, null, 4)}\n`, 'utf8')
      return { ok: true, size }
    } catch (error) {
      return { ok: false, error: String(error?.message ?? error) }
    }
  ...
  function uiStateRoute(req, res) {
    ...
    json(res, 200, {
      ...
      petPid: petRunning() ? petChild.pid : null,
      petOnline: petOnline(),
  ...
  async function writeStateFile(port) {
    const payload = {
      ...
      diagnostics: {
        ...
        petChildPid: petRunning() ? petChild.pid : null,
