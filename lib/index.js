...
export function apply(ctx, config = {}) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    const beforeLabel = displayName(record)
    switch (event.type) {
      case 'session/title':
        // 会话标题走独立事件下发（session.header 里没有 title 字段），不处理就只剩 id 可显示
        if (typeof event.data?.title === 'string' && event.data.title !== '') record.title = event.data.title
        break
      case 'turn/start':
      ...
      case 'turn/end': {
        ...
        if (!failed && durationMs >= (options.celebrateMinTurnMs ?? 8_000)) {
          push('celebrate', { sessionId: id, title: displayName(record), durationMs })
        }
        break
      }
      default:
    ...
    // 状态变了、或者标题刚下来，都要立刻推给桌宠（标题事件不改状态，不推的话桌宠要等下一次轮询才看到名字）
    if (before !== record.state || beforeLabel !== displayName(record)) push('session', publicSession(record))
  ...
  /** 目录名：取路径最后一段（DSH 侧同名口径，用于没有标题时的显示名） */
  ...
  /** 界面上只出现会话名：标题 → 工作目录名 → 未命名会话（会话 id 是内部标识，不下发给桌宠显示） */
  function displayName(record) {
    if (typeof record.title === 'string' && record.title !== '') return record.title
    const base = workspaceName(record.cwd)
    return base === '' ? '未命名会话' : base
  }
