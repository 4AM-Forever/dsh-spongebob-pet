...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PetSettingsTab() {
      ...
      const current = SIZE_OPTIONS.find((option) => option.key === ((state && state.petSize) || 'large')) || SIZE_OPTIONS[0]
        ...
        h('div', { style: styles.card },
          ...
          h('div', { style: styles.row },
            ...
            SIZE_OPTIONS.map((option) => h('button', {
              ...
              disabled: busy,
              onClick: () => act('/pet/ui/size', { size: option.key }),
            }, option.label)),
            h('span', { style: styles.sizeHint }, '当前：' + current.label + '（' + current.hint + '）'),
          ),
