...
| **桌宠端到端 24 项**（多会话气泡、点桌宠弹出/收起会话面板、面板显示会话名且不出现会话 id、挪动时面板自动收起、**副屏不被拽回主屏**、丢出屏幕能自己拉回、庆祝有过程且时长对得上、运行期无异常） | `powershell -File tools\e2e-pet-features.ps1`（临时副本 + `test/stub-bridge.mjs` 假桥接，不碰正在跑的那只桌宠） | 24/24 通过 |
