...
| **桌宠端到端 14 项**（多会话气泡、点桌宠弹出/收起会话面板、挪动时面板自动收起、庆祝动画、运行期无异常） | `powershell -File tools\e2e-pet-features.ps1`（临时副本 + `test/stub-bridge.mjs` 假桥接，不碰正在跑的那只桌宠） | 14/14 通过（连跑 3 次） |
